...
export class Character {
  ...
  steerWalls(dir, time) {
    ...
    for (const w of state.walls) {
      ...
      const hT = w.halfT + rad;
      if (Math.abs(lx) < hW && Math.abs(lz) < hT) {
        if (this.isOnGround && Math.random() < 0.35) this.jump();
        ...
        if (dx*dx + dz*dz < 25) {
          if (p.topY > this.position.y + 0.2 && p.topY < this.position.y + 11) {
            if (this.isOnGround && Math.random() < 0.2) {
              this.jump();
            } else if (this.jumps < state.maxAllowedJumps && this.velocity.y < 3 && p.topY > this.position.y + 2 && Math.random() < 0.15) {
              this.jump();
            }
          }
        }
